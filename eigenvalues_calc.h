...
template <size_t DIM>	// function should receive eigenvalues
Solution_type stability_investigation(const std::array< std::array<double, DIM>, DIM >& monodromy_mat) {
	Eigen::Matrix< double, DIM, DIM > eig_mat;
	for (size_t i = 0; i < DIM; ++i) {
		for (size_t j = 0; j < DIM; ++j) {
			eig_mat(i, j) = monodromy_mat[i][j];
	...
	} else if ( isapprox(eigenvals(0).real(), -1.) ) {	// double -1
		return Solution_type::double_minus_one;
	} else {
		return Solution_type::double_plus_one;	// double +1
	}
